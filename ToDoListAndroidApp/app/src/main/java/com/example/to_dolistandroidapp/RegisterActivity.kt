...
class RegisterActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSignUp.setOnClickListener {
            ...
            } else if (userNameSignUp.text.toString().length < 5) {
                userNameSignUp.error = "User Name must be 5 or more characters in length"
            }
            ...
            if (userNameSignUp.error.isNullOrEmpty() && emailSignUp.error.isNullOrEmpty() && passwordSignUp.error.isNullOrEmpty()) {
                val encodedPassword = Base64.getEncoder().encodeToString(passwordSignUp.text.toString().toByteArray())
                val encodedEmail = Base64.getEncoder().encodeToString(emailSignUp.text.toString().toByteArray())
                signUp(userNameSignUp.text.toString(), encodedEmail, encodedPassword, API)
            }
        }
    }

    private fun signUp(userName: String, email: String, password: String, API: RetrofitInterface) {
